...
def model_line(domain, w, b, visible=True, meta=None):
    x = torch.linspace(domain[0][0], domain[0][1], 100)
    y = (w[0] * x + b) / -w[1]
    return go.Scatter(
        x=x,
        y=y,
...
def model_traces(frame: AnimationFrame, view: ParameterView, animation: Animation) -> list[go.Scatter3d]:
...
def weights_traces(frame: AnimationFrame, animation: Animation, parameter: Union[int, str]) -> list[go.Scatter]:
    return _model_traces(
        view=animation.get_param_view(frame, parameter=parameter),
        frame=frame,

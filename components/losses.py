from base import Animation, AnimationFrame, LayoutComponent, ParameterView

import plotly.graph_objects as go
import torch

from base import Plot2DFrame
...
class LossesComponent(LayoutComponent):
    ...
    def create_component(self, frame: AnimationFrame, view: ParameterView):
        return []

    def update_component(self, frame: AnimationFrame, view: ParameterView):
        return [Plot2DFrame(data=losses_table(view, frame, self.animation))]
...
class CostComponent(LayoutComponent):
    ...
    def update_component(self, frame: AnimationFrame, view: ParameterView):
        return [Plot2DFrame(data=cost_table(view, frame, self.animation))]
...
def cost_table(frame: AnimationFrame, view: ParameterView, animation: Animation, meta=None):
    X = frame.X
    m = X.size(0)
    preds = view.preds
    targets = view.targets
